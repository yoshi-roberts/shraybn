  ...
  Central directory structure:
	...
	for fileID, fileInfo in pairs(filesInfos) do
      ...
      MUST be encoded using UTF-8. (see APPENDIX D)]]
		...
		centeralDirectorySize = centeralDirectorySize
			+ 46
			+ #fileInfo.fileName
			+ #fileInfo.extraField
			+ #fileInfo.comment
...
function zapi.newZipWriter(zipFile)
	...
	function writer.addFile(fileName, fileData, modTime, extraField, fileComment, attributes)
		...
		local fileInfo =
			writeFile(zipFile, fileName, fileData, modTime, extraField, fileComment, attributes)
...
function zapi.createZip(path, zipFile)
	...
	local function index(dir)
		...
		local dirInfo = nativefs.getInfo(dir)

		if dirInfo.type == "file" then
			-- local fileData = love.filesystem.read(dir)
			local fileData = nativefs.read(dir)
		...
		elseif dirInfo.type == "directory" then
			...
			for id, item in ipairs(nativefs.getDirectoryItems(dir)) do
				index(dir .. "/" .. item)
			end
		end
	end
...
function zapi.writeZip(path, destination)
	...
	local zipFile, zipFileErr = nativefs.newFile(destination, "w")
	if not zipFile then
		return false, "Failed to open destination zip file: " .. tostring(zipFileErr)
	end
