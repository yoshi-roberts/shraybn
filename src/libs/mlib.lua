...
local function getPerpendicularSlope(...)
	...
	elseif checkFuzzy(slope, 0) then
		return false
	end -- Horizontal lines become vertical.
	return -1 / slope
end
...
local function getClosestPoint(perpendicularX, perpendicularY, ...)
	...
	elseif slope == 0 then -- Horizontal line
		...
		local perpendicularIntercept =
			getIntercept(perpendicularX, perpendicularY, perpendicularSlope)
		x, y = getLineLineIntersection(
			slope,
			intercept,
			perpendicularSlope,
...
local function getLineSegmentIntersection(x1, y1, x2, y2, ...)
	...
	local slope1, intercept1
	local slope2, intercept2 =
		getSlope(x1, y1, x2, y2), getIntercept(x1, y1, x2, y2)
	local x, y
...
local function getSegmentSegmentIntersection(x1, y1, x2, y2, x3, y3, x4, y4)
	local slope1, intercept1 =
		getSlope(x1, y1, x2, y2), getIntercept(x1, y1, x2, y2)
	local slope2, intercept2 =
		getSlope(x3, y3, x4, y4), getIntercept(x3, y3, x4, y4)
	...
	if
		x
		and checkSegmentPoint(x, y, x1, y1, x2, y2)
		and checkSegmentPoint(x, y, x3, y3, x4, y4)
	then
		return x, y
	end
...
local function round(number, place)
	...
	if checkFuzzy(high, number) then
		...
		if highDifferance <= lowDifferance then
			returnValue = high
		else
			returnValue = low
		end
...
local function getCircleLineIntersection(
	circleX,
	circleY,
	radius,
	x1,
...
local function getCircleSegmentIntersection(
	circleX,
	circleY,
	radius,
	x1,
	...
	if
		isPointOnCircle(x1, y1, circleX, circleY, radius)
		and isPointOnCircle(x2, y2, circleX, circleY, radius)
	then -- Both points are on line-segment.
		return "chord", x1, y1, x2, y2
	end

	if slope then
		if
			checkCirclePoint(x1, y1, circleX, circleY, radius)
			and checkCirclePoint(x2, y2, circleX, circleY, radius)
		then -- Line-segment is fully in circle.
			return "enclosed", x1, y1, x2, y2
		elseif x3 and x4 then
			if
				checkSegmentPoint(x3, y3, x1, y1, x2, y2)
				and not checkSegmentPoint(x4, y4, x1, y1, x2, y2)
			then -- Only the first of the points is on the line-segment.
				return "tangent", x3, y3
			elseif
				checkSegmentPoint(x4, y4, x1, y1, x2, y2)
				and not checkSegmentPoint(x3, y3, x1, y1, x2, y2)
			then -- Only the second of the points is on the line-segment.
				...
				if
					checkSegmentPoint(x3, y3, x1, y1, x2, y2)
					and checkSegmentPoint(x4, y4, x1, y1, x2, y2)
				then
		...
		if bottomY ~= topY then -- Not a tangent
			if
				checkSegmentPoint(topX, topY, x1, y1, x2, y2)
				and checkSegmentPoint(bottomX, bottomY, x1, y1, x2, y2)
			then
...
local function getCircleCircleIntersection(
	circle1x,
	circle1y,
	radius1,
	circle2x,
	...
	local a = (radius1 * radius1 - radius2 * radius2 + length * length)
		/ (2 * length)
	...
	if
		not validateNumber(p3x)
		or not validateNumber(p3y)
		or not validateNumber(p4x)
		or not validateNumber(p4y)
	then
		return "inside"
	end

	if
		checkFuzzy(length, radius1 + radius2)
		or checkFuzzy(length, math.abs(radius1 - radius2))
	then
...
local function isCircleCompletelyInsideCircle(
	circle1x,
	circle1y,
	circle1radius,
	circle2x,
	...
	local Type = getCircleCircleIntersection(
		circle2x,
		circle2y,
		circle2radius,
		circle1x,
...
local function isSegmentCompletelyInsideCircle(
	circleX,
	circleY,
	circleRadius,
	x1,
	...
	local Type = getCircleSegmentIntersection(
		circleX,
		circleY,
		circleRadius,
		x1,
...
local function getPolygonLineIntersection(x1, y1, x2, y2, ...)
	...
	for i = 1, #input, 2 do
		if input[i + 2] then
			local x1, y1, x2, y2 = getLineSegmentIntersection(
				input[i],
				input[i + 1],
				input[i + 2],
				input[i + 3],
...
local function checkPolygonPoint(px, py, ...)
	...
	if greatest < px then
		return false
	end -- Speeds up process and prevents false positives (segment will always intersect).
	if least > px then
		return false
	end
	...
	for i = 1, #points, 2 do
		...
		if
			getSegmentSegmentIntersection(px, py, greatest, py, x1, y1, x2, y2)
		then
			count = count + 1
		end
	end
...
local function getPolygonCircleIntersection(x, y, radius, ...)
	...
	for i = 1, #input, 2 do
		if input[i + 2] then
			local Type, x1, y1, x2, y2 = getCircleSegmentIntersection(
				x,
				y,
				radius,
				input[i],
...
local function isPolygonCompletelyInsidePolygon(polygon1, polygon2)
	for i = 1, #polygon1, 2 do
		...
		local x2, y2 =
			polygon1[i + 2] or polygon1[1], polygon1[i + 3] or polygon1[2]
...
local function isPolygonCompletelyInsideCircle(
	circleX,
	circleY,
	circleRadius,
	...
	...
	local function isDistanceLess(px, py, x, y, circleRadius) -- Faster, does not use math.sqrt
		...
		return distanceX * distanceX + distanceY * distanceY
			< circleRadius * circleRadius -- Faster. For comparing distances only.
	end
...
-- circleX, circleY, circleRadius, polygonPoints
local function isCircleCompletelyInsidePolygon(
	circleX,
	circleY,
	circleRadius,
	...
